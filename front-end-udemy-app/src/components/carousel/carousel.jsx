...
export const Carousel = ({ activeNumber, dataSet }) => {
    ...
    const calcActivePage = () => {
        const maxScrollLeft = bodyScroll.current.scrollWidth - bodyScroll.current.clientWidth;
        const minRange = parseInt(maxScrollLeft / numberPage.length) * (pagiActive - 1);
        const maxRange = parseInt(maxScrollLeft / numberPage.length) * (pagiActive);
        if (bodyScroll.current.scrollLeft > maxRange) {
            setPagiActive(((pagiActive + 1) > numberPage.length) ? numberPage.length : pagiActive + 1);
        };
            ...
            <div className='pagination'>
                {
                    ...
                    numberPage.map((item) => {
                        return (
                            <div data-id={item.index} className={`pagination__item item--${(item.index === pagiActive) ? 'active' : 'disabled'}`}
                                onClick={handleClickPagination}></div>
                        )
                    })
                }
            </div>
